...
def format_version(build_number):
    # Extract the first two digits to form the version number
    return f"{build_number[0]}.{build_number[1]} {build_number}"

def save_versions_to_txt(version_numbers, output_file_path):
    with open(output_file_path, 'w') as file:
        for version in version_numbers:
            formatted_version = format_version(version)
            file.write(f"{formatted_version}\n")
...
if __name__ == "__main__":
    ...
    if html_content:
        ...
        save_versions_to_txt(version_numbers, output_file_path)
